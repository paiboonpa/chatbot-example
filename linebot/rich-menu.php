  ...
  if ($event instanceof \LINE\LINEBot\Event\MessageEvent) {
    if($event instanceof \LINE\LINEBot\Event\MessageEvent\TextMessage) {
      ...
      if($event->getText() === 'create') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(createNewRichmenu($channelToken)));
      }
      else if($event->getText() === 'list') {
        $result = getListOfRichmenu($channelToken);
        ...
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(unlinkFromUser($channelToken, $event->getUserId())));
      }
      else if($event->getText() === 'check') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(checkRichmenuOfUser($channelToken, $event->getUserId())));
      }
      else if(substr($event->getText(),0, 8) === 'upload::') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(uploadRandomImageToRichmenu($channelToken, substr($event->getText(), 8))));
      }
      else if(substr($event->getText(),0, 8) === 'delete::') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(deleteRichmenu($channelToken, substr($event->getText(), 8))));
      }
      else if(substr($event->getText(),0, 6) === 'link::') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(linkToUser($channelToken, $event->getUserId(), substr($event->getText(), 6))));
      }
      else {
